...
void main()
{
  ...
  float noise = snoise(vec3(noiseCoord.x + time * 0.1, noiseCoord.y, time * 0.3));
  ...
  for (int i = 0; i < 4; i++) {
    ...
    float noiseFlow = 0.05 + float(i) * 0.01;
    float noiseSpeed = 0.1 + float(i) * 0.01;
    float noiseSeed = float(i) * 10.0;
